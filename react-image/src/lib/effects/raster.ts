...
export const applyRasterEffect = (
  ctx: CanvasRenderingContext2D,
  img: HTMLImageElement | HTMLCanvasElement,
  width: number,
  height: number,
  style: 'dots' | 'lines-horizontal' | 'lines-vertical',
  config: RasterConfig
) => {
  ...
  if (style === 'dots') {
    applyDotRaster(ctx, redChannel, 'red', config, 0, 0);
    applyDotRaster(ctx, greenChannel, 'green', config, 1, 1);
    applyDotRaster(ctx, blueChannel, 'blue', config, -1, -1);
  } else {
    const isVertical = style === 'lines-vertical';
    applyLineRaster(ctx, redChannel, 'red', config, isVertical, 0, 0);
    applyLineRaster(ctx, greenChannel, 'green', config, isVertical, 1, 1);
    applyLineRaster(ctx, blueChannel, 'blue', config, isVertical, -1, -1);
  }
